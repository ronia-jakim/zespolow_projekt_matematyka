...
Liniowość przestrzeni stycznych jest wykorzystywana w celu dodawania rozmaitościom dodatkowej struktury, mianowicie pozwala na definiowanie na wiązce stycznej $M$ odpowiednika iloczynu skalarnego, nazywanego *tensorem metrycznym*. 
...
Ponieważ $g$ jest odwzorowaniem liniowym na $T_p M$ dla każdego $p in M$, to zapisuje się ono macierzą, którą nazwiemy $g$. Jej wyraz odpowiadający $g(diff phi_i, diff phi_j)$ będziemy oznaczać $g_(i,j)$.

Warto zaznaczyć, że mając bazę dualną ${d phi^i}$ do ${ diff phi_i}$, tensor metryczny możemy zapisać jako
...
$ g = c^2 d tau^2 = -(1 - (r_s) / r ) c^2d t^2 + ( 1 - (r_s) / r )^(-1) d r^2 + r^2(d theta^2 + sin^2(theta) d phi^2) $
